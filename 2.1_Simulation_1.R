...
Cn = foreach(i = 1:1, .combine = rbind,.packages = c("geeM", "tidyverse","brm","gesttools")) %dorng% {
  {
    ...
    X = cbind(1,A1, L1,U)
    alpha.true = c(-1, psi1.asoc, 0.5, 0.5)
    p0p1.true = getProbRR(Z2 %*% psi2.true,X %*% alpha.true)
    pA.true = p0p1.true[,1]
    pA.true[A2==1]=p0p1.true[A2==1,2]
    Y = rbinom(n, 1, pA.true)
  }
  H1 = Y*exp(-Z2 %*% psi2.true * A2)
  pars1 = LSEst(Y,A1,Z1,W1)
  PS1.model = glm(A1~W1,family = binomial(link = "logit"))
  PS1 = predict(PS1.model,type="response")
  psi1 = DREst(Y,A1,Z1,W1,pars1,PS1)
  
  PSI1 = c(nonDR = pars1$psi, DR = psi1$minimum, ls = pars1$value, sol = psi1$objective)
...
# specify the seed value
.Random.seed = seed_summary$SIM1_pos
...
result = foreach(i = 1:10, .combine = rbind,.packages = c("geeM", "tidyverse","brm","gesttools")) %dorng% {
  ...
  H1 = Y*exp(-psi2$minimum*A2)
  pars1.mis = LSEst(H1,A1,Z1,W1.mis)
  PS1.model = glm(A1~W1,family = binomial(link = "logit"))
  PS1 = predict(PS1.model,type="response")
  psi1 = DREst(H1,A1,Z1,W1.mis,pars1.mis,PS1)
  ...
  #psi2 = DREst_re(Y,A2,Z2,W2.mis,pars2.mis,psi2,PS2.mis)
  res[1,13] = psi2$minimum
  
  H1 = Y*exp(-psi2$minimum*A2)
  pars1.mis = LSEst(H1,A1,Z1,W1.mis)
  psi1 = DREst(H1,A1,Z1,W1.mis,pars1.mis,PS1.mis)
  #psi1 = DREst_re(H1,A1,Z1,W1.mis,pars1.mis,psi1,PS1.mis)
